...
qsa('nav ul li a').forEach(link => {
  link.addEventListener('click', (e) => {
    if (window.innerWidth <= 900) {
      // Use standard toggle logic to close
      const nav = qs('nav ul')
...
qs('.view', lightbox).appendChild(lightboxCounter)
const lightboxLoader = document.createElement('div')
lightboxLoader.className = 'lightbox-loader'
qs('.view', lightbox).appendChild(lightboxLoader)
...
function setupLightbox() {
  qsa('#galleryGrid .card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      // Fallback for single image cards not using data-images
      if (!imagesAttr && card.querySelector('img')) {
...
function updateLightboxImage() {
  ...
  // Visual cues for navigation buttons
...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mgoooeng', {
        method: 'POST',
        body: formData,
